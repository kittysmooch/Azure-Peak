...
/datum/unit_test/nodupe_smeltresult/Run()
	...
	for(var/datum/anvil_recipe/recipe as anything in GLOB.anvil_recipes)
		...
		// Uses TEST_NOTICE to avoid failing the entire test suite
		// If we create more items than we consume with the same smelt result, that's material duping
		if(target_item_count > built_item_count && built_item_count > 0)
			TEST_FAIL("Recipe '[recipe.name]' creates [target_item_count]x [created_item] (smelt: [smelt_result]) but only requires [built_item_count] items with same smelt result.")
		// If we create items but require NO items with same smelt result, that's transmutation (creating from nothing or different material)
		else if(target_item_count > 0 && built_item_count == 0)
			TEST_FAIL("Recipe '[recipe.name]' creates [target_item_count]x [created_item] (smelt: [smelt_result]) but requires 0 items with same smelt result.")
