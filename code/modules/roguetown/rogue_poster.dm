//ports posters from tg to support wanted posters. we can probably find clever reuses for these if people wanna decorate their spaces or something
 ...
 */
/obj/item/poster
	name = "poorly coded flyposting"
	desc = "You probably shouldn't be holding this."
	icon = 'icons/roguetown/misc/posters.dmi'
	force = 0
...
/obj/structure/sign/poster
	name = "flyposting"
	var/original_name
	desc = "A parent item. That means you shouldn't be seeing this in game."
	icon = 'icons/roguetown/misc/posters.dmi'
	anchored = TRUE
	var/ruined = FALSE
	var/random_basetype
...
/obj/structure/sign/poster/Initialize(mapload)
	...
	if(!ruined)
		...
		desc = "A paper posting adhered to the wall with saigaskin glue. [desc]"
...
/obj/structure/sign/poster/proc/check_tearability(mob/user)
	if(ruined)
		to_chat(user, span_notice("already ruined!"))
		return FALSE
	return TRUE
...
/turf/closed/proc/place_poster(obj/item/poster/rolled_poster, mob/user)
	...
	for(var/obj/contained_object in contents) //Let's see if it already has a poster on it or too much stuff
		if(istype(contained_object, /obj/structure/sign/poster))
			to_chat(user, span_notice("no room!"))
			return
		stuff_on_wall++
		if(stuff_on_wall == 3)
			to_chat(user, span_notice("no room!"))
	...
	to_chat(user, span_notice("hanging poster..."))
